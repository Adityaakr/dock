...
<Callout type="info" title="The one-command creator">
  Any assistant can fall back to `create-basket.sh`, served from this site. It
  resolves each market on Polymarket, requests gas from the voucher backend,
  creates the basket and prints the link:

  ```bash
  curl -fsSL https://docs.polybaskets.xyz/create-basket.sh | bash -s -- \
    --name "BTC Strength" --desc "Bitcoin holds above 74k" \
