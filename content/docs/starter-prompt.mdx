...
Paste this into Claude or ChatGPT, answer three questions, say "go", and the
assistant finds the markets, creates the basket on Vara mainnet and hands you
the link to open your position. The prompt is self-contained: every contract
address and the contract interface are included or fetched from this site.

<Tabs items={['Claude (claude.ai)', 'ChatGPT']}>

<Tab value="Claude (claude.ai)">

Paste the prompt into a new chat on **claude.ai**. Claude runs the commands in its own sandbox (install `vara-wallet`, request a gas voucher, fetch live markets, create the basket) and reports back with the basket link. No terminal on your side, nothing to install, no repo access needed: the prompt carries every address and fetches the contract interface from this site.
...
Paste the prompt into **ChatGPT** with **agent mode** enabled (the mode with a virtual computer). The plain chat sandbox has no internet access and cannot reach Vara or Polymarket. If ChatGPT reports it cannot install packages or reach the network, ask it to output the exact commands instead and run them in your own terminal; they are all in the prompt.
...
<Callout type="info" title="What the assistant will ask you">
  Your conviction or theme (or "surprise me"), how many markets (2 to 5), and
  the time horizon (48h for fast resolution, or any). Answer in one message,
  review the proposed basket, reply "go".
</Callout>
